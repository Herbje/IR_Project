...
import math
import random
from run_experiments import monot5, index_msmarco_eval, Monot5ModelType
import numpy as np
...
class LIRME_posting:
    ...
    def __init__(self, reranker, query, document, monot5_v=Monot5ModelType.UNBIASED, m=200, h=0.75, chunk_size=5, v=0.1):
...
if __name__ == '__main__':
    ...
    for i, (q_ind, q) in tqdm.tqdm(enumerate(msmarco_eval_dataset.get_topics().iterrows())):
        ...
        for r, (res_ind, res_d) in enumerate((bm25 % 5).search(q['query']).iterrows()):
            lirme_inst = LIRME_posting('bm25', q, res_d, m=250, h=0.75)
